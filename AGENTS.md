...
**Current phase:** 0 — archive and documentation. No application code exists yet. Sections below marked _(Phase N)_ describe the target state and are not yet true.
...
- **Prefer the framework's own primitive over a hand-written one.** Before writing a helper, check whether Payload or Next already provides it — Payload in particular ships things that are easy to miss (`slugField`, `imageSizes`, `formatOptions`, access-control helpers). A built-in has handled edge cases you have not thought of yet, and it is one less thing to maintain. If you do write your own, say in a comment what you checked and why it did not fit, so the next person can re-evaluate on upgrade instead of assuming it was an oversight.
