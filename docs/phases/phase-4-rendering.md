...
- **Doodle layer (not yet built) — three landmines already hit once in prototyping.** Placement must use a seeded PRNG keyed on section id, never `Math.random()` — otherwise server and client render different layouts, React warns on hydration mismatch, and marks visibly jump on load. Parallax needs three depth wrappers per zone, not one transformed node per mark — transforming ~125 individual SVGs with `will-change` each blew up the compositor and rendered the page black. The layer is decorative (`aria-hidden="true"`, `pointer-events: none`) and must cap node count per zone; the scroll listener itself must not run under `prefers-reduced-motion`, not just its visible effect. Tokens: `DESIGN.md`'s Doodle layer section.
