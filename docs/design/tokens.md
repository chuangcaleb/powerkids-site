...
**Type.** Fluid scale via Utopia, two regimes: steps `-2` to `1` (caption through h5/h4) compound from a tight ratio; steps `2`–`4` (h3/h2/h1) are explicit hand-picked pairs, since compounding one ratio that far inverts at one end. Two families: `--font-display` is `--font-bricolage-grotesque` (partial variable — `wdth` pinned at build time (maxed, already as wide as the typeface goes), `opsz`/`wght` left variable in the font file, italic dropped, no Marker Felt/Comic Sans ever); `--font-body` is `--font-figtree` (`next/font/google`, weights 400–700, no axis worth pinning). `font-optical-sizing: auto` is set globally so Bricolage's `opsz` tracks each element's rendered size automatically — a no-op on Figtree. Headings override that with a fixed cut: `--opsz-display` (`opsz`, via `font-variation-settings`), `--weight-display` (`font-weight`), and `--tracking-display` (`letter-spacing`) — all in `type.css`, matching the reference exploration proto's pinned Bricolage settings (`_reference/design-exploration/proto/panel3.html`). Values decided in `DESIGN.md`'s `type.displayOpsz`/`displayWeight`/`displayTracking`; tune live in devtools, then write back. `Button` reads `--font-display` (see `docs/design/components.md`); a coloured pill/badge reads `--font-body` at a smaller step — neither needs a third face.
