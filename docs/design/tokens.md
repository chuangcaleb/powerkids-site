...
**Type.** Fluid scale via Utopia, two regimes: steps `-2` to `2` (caption through h5/h4) compound from a tight ratio; steps `3`–`5` (h3/h2/h1) are explicit hand-picked pairs, since compounding one ratio that far inverts at one end. One family, `--font-bricolage-grotesque` (Bricolage Grotesque, partial variable — `wdth` pinned to 90 at build time, `opsz`/`wght` left variable, italic dropped, no Marker Felt/Comic Sans ever), used for both `--font-display` and `--font-body`. No second webfont. `font-optical-sizing: auto` is set globally so `opsz` tracks each element's rendered size automatically. No Pill component's text needs a third face — a coloured pill/badge reads the same family at a smaller step.
