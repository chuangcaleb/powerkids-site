...
4. **Contrast is checked on the pair actually rendered, not the token in isolation.** `color.red` on `color.cream` must clear 4.5:1 for any body-sized text; verified pairs so far: ink-on-cream, charcoal-on-cream, white-on-red, white-on-blue. Add a line here when a new pair enters production use.
...
Fluid scale (Utopia-generated, same mechanism as type), consumed by the layout primitives through their own custom properties (`--flow-space`, `--cluster-gap`, `--grid-item-min`, etc.) — components never set spacing directly.
